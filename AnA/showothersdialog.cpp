...
#include "ui_showothersdialog.h"

ShowOthersDialog::ShowOthersDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ShowOthersDialog)
{
    ui->setupUi(this);
  //  QMessageBox::information(NULL,"",curCheckID,QMessageBox::Yes);
    ShowAnswerList(curCheckID);
    ShowQuestionList(curCheckID);
}

ShowOthersDialog::~ShowOthersDialog()
{
    delete ui;  
...
void ShowOthersDialog::ShowAnswerList(QString id){
    vector<User>::iterator useriter;//获得一个指向Answer类数组的迭代器
    useriter=searcher->searchUserById(id);
    ui->AnswerTable->setColumnCount(ColNum-1); //列数
    ui->AnswerTable->setRowCount(useriter->GetUserAnswerNum());//行数
  ...
  vector<Answer>::iterator qiter;//获得一个指向Answer类数组的迭代器
       for(int i=0;i < useriter->GetUserAnswerNum(); i++)
       {
            //通过用户ID从所有回答中找到自己的回答并输出
            qiter=searcher->searchAnswerById(useriter->answerQuestionID[i]);
...
void ShowOthersDialog::ShowQuestionList(QString id)
{
    vector<User>::iterator useriter;//获得一个指向Answer类数组的迭代器
    useriter=searcher->searchUserById(id);
ui->QuestionTable->setColumnCount(ColNum); //列数
ui->QuestionTable->setRowCount(useriter->GetQuestionNum());//行数
...
vector<Question>::iterator iter;//获得一个指向question类数组的迭代器
   ...
   for(int i=0;i < useriter->GetQuestionNum(); i++)
   {
        //通过问题ID从所有回答中找到自己的会回答并输出
        iter=searcher->searchQuestionById(useriter->askQuestionID[i]);
